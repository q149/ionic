...
.controller('AppCtrl', function($scope, $ionicModal, $timeout, $localStorage, $ionicPlatform, $cordovaCamera) {
  ...
  .controller('MenuController', ['$scope', 'dishes', 'favoriteFactory', 'baseURL', '$ionicListDelegate', '$ionicPlatform',
    ...
    $scope.dishes = dishes;
    //console.log('mijne menu::' + $scope.dishes);
  ...
  .filter('favoriteFilter', function() {
    ...
    return function(dishes, favorites) {
      ...
      for (var i = 0; i < favorites.length; i++) {
         console.log('favorites' + typeof favorites[i].id);
        for (var j = 0; j < dishes.length; j++) {
           console.log('dingen' + typeof dishes[j].id);
          if (parseInt(dishes[j].id) === parseInt(favorites[i].id))
            out.push(dishes[j]);
        }
      }
      return out;
